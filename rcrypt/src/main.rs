...
fn read_armored(armored: String) -> Result<Vec<u8>, HedwigError> {
    let mut split = armored.split('=');
    let data_str = split.next();
    let crc_str = split.last();
    if data_str.is_none() {
    ...
    let data_bytes = try!(data_str.unwrap().from_base64());
    let crc_bytes = try!(crc_str.unwrap().from_base64());
    let mut crc = 0;
    for b in crc_bytes {
        crc = crc << 8;
        crc |= b as u32;
    }
    let data_crc = calc_crc(&data_bytes);
    if crc != data_crc {
